...
export default function GoogleSignInButton({
    ...
    const handleCredentialResponse = async (response: any) => {
      ...
      try {
        ...
        if (data.success) {
          ...
          throw new Error(data.message || `Google ${text} failed`);
        }
      } catch (error: any) {
        const errorMessage = error.message || `Google ${text} failed`;
  ...
  }, [disabled, onSuccess, onError, router, text]);
  ...
  if (disabled || isLoading) {
    return (
      <button
        ...
        {isLoading ? `${capitalizedText}...` : `${capitalizedText}`}
      </button>
    );
  }
